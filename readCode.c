...
int *completed;				/* number of bytes copied */
{
	...
	*completed = 0;
	...
	block_t start_pos, track_off, cur_block;
	unsigned int b_ahead;

	cur_block = bp->lmfs_blocknr;
	track_off = cur_block % 16;		   /* block mod 16 = resto da divisao por 16, descobre qual e a pos do bloco na trilha */
	start_pos = cur_block - track_off; /* Subtrai a pos do bloco na trilha do bloco para comecar no inicio da trilha */

	for (block_t b_index; b_index < start_pos + 16; b_index++)
	{
		if (b_index != cur_block)
		{
			get_block(dev, b_index, PREFETCH);
